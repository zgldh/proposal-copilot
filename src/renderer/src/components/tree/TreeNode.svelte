<script lang="ts">
  ...
  let { node, depth = 0, oncontextmenu, onrename }: Props = $props()
  ...
  {#if isExpanded && node.children.length > 0}
    <div class="children">
      {#each node.children as child (child.id)}
        <TreeNode node={child} depth={depth + 1} {oncontextmenu} {onrename} />
      {/each}
    </div>
  {/if}
</div>
...
<style>
  ...
  .node-content:hover { background: var(--color-background-soft); }
  .node-content.selected { background: var(--ev-c-primary); color: white; }
  .node-content.dragging { opacity: 0.5; }
  
  .drop-before { border-top-color: var(--ev-c-primary); }
  .drop-after { border-bottom-color: var(--ev-c-primary); }
  .drop-inside { background: rgba(74, 144, 217, 0.2); }

  .expand-icon { width: 20px; border: none; background: none; color: inherit; cursor: pointer; font-size: 0.7rem; }
  ...
  .badge { background: rgba(0,0,0,0.2); border-radius: 10px; padding: 0 6px; font-size: 0.75rem; margin-left: 8px; }
  input { font-size: inherit; padding: 2px 4px; border-radius: 2px; border: none; width: 100%; color: black; }
</style>
