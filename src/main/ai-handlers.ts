...
export function setupAIHandlers(_mainWindow: BrowserWindow) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        // Log User Message
        chatLogger.log(projectPath, 'user', message)

        // Create checkpoint before AI operation
        console.log('[IPC] ai:process-message: creating snapshot')
        const currentProject = await projectService.loadProject(projectPath)
        // Update history in the project object for the snapshot
        currentProject.chat_history = history
        await projectService.createSnapshot(projectPath, 'Before AI Operation', currentProject)
